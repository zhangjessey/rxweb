...
/**
 * A {@link Subject} implementation to be used by {@code HttpClientResponse} and {@code HttpServerRequest}.
 *
 * <h2>Unicast</h2>
...
public final class UnicastContentSubject<T> extends Subject<T, T> {
    ...
    private UnicastContentSubject(final State<T> state, long noSubscriptionTimeout, TimeUnit timeUnit, Scheduler scheduler) {
        super(new OnSubscribeAction<T>(state));
        this.state = state;
